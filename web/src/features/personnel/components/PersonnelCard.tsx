...
const AVATAR_PALETTES = [
  ...
  },
  {
    ...
    ring: 'border-[var(--status-success-border)]',
  },
  {
    bg: 'bg-[var(--status-info-soft)]',
    text: 'text-[var(--status-info)]',
    ring: 'border-[var(--status-info-border)]',
  },
  {
    bg: 'bg-[var(--status-warning-soft)]',
    text: 'text-[var(--status-warning)]',
    ring: 'border-[var(--status-warning-border)]',
  },
  {
    bg: 'bg-[var(--status-danger-soft)]',
    text: 'text-[var(--status-danger)]',
    ring: 'border-[var(--status-danger-border)]',
  },
]
    ...
    <article
      ...
      className={`frosted-glass-interactive group relative flex min-w-0 cursor-pointer flex-col justify-between rounded-2xl border p-3.5 text-left shadow-xs transition-all duration-200 ease-out hover:-translate-y-1 hover:shadow-lg focus-visible:ring-2 focus-visible:ring-[var(--ring)] focus-visible:outline-none ${
        ...
        isSelected
          ...
          : 'z-1 border-[var(--border)] bg-[var(--bg-surface)] hover:border-[var(--border-strong)]'
      }`}
    >
      {/* 顶部微细环境高光线（未选中时展示，选中时让出纯正蓝色高光） */}
      {!isSelected && (
        <div className="pointer-events-none absolute inset-x-0 top-0 h-px rounded-t-2xl bg-gradient-to-r from-transparent via-[var(--border-strong)] to-transparent" />
      )}

      {/* ── 核心主体：左侧黄金人像相框 + 右侧饱满信息流 ── */}
      <div className="flex min-w-0 items-start gap-3.5">
        {/* ── 1. 左侧：独立 4:5 肖像相框 (严丝合缝，100% 完整展示人像，无左右黑边) ── */}
        <div className="relative h-36 w-28 shrink-0 overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--bg-secondary)] shadow-inner sm:h-40 sm:w-32">
          {avatarUrl ? (
            <img
              src={avatarUrl}
              alt={person.name}
            ...
            <div
              className={`flex h-full w-full flex-col items-center justify-center ${palette.bg} p-2 text-center`}
            >
              <div
                className={`flex h-12 w-12 items-center justify-center rounded-xl border ${palette.ring} bg-[var(--bg-surface)]/40 shadow-inner backdrop-blur-sm`}
              >
            ...
            <div className="flex items-center justify-between gap-1.5">
              ...
              <div className="relative" ref={menuRef} onClick={(e) => e.stopPropagation()}>
                ...
                {menuOpen && (
                  <div
                    role="menu"
                    className="lens-glass absolute top-full right-0 z-50 mt-1.5 w-max min-w-[160px] rounded-xl border border-[var(--border-strong)] bg-[var(--bg-surface-solid)]/98 p-1.5 whitespace-nowrap shadow-2xl backdrop-blur-xl"
                  >
                    <button
                      type="button"
                      role="menuitem"
                    ...
                    {!isSaturated && onAddFace && (
                      <button
                        ...
                        className="flex min-h-7 w-full items-center gap-2 rounded-lg px-2 text-xs font-medium text-[var(--status-success)] transition-colors hover:bg-[var(--status-success-soft)]"
                      >
                    ...
                    <button
                      ...
                      className="flex min-h-7 w-full items-center gap-2 rounded-lg px-2 text-xs font-medium text-[var(--status-danger)] transition-colors hover:bg-[var(--status-danger-soft)]"
                    >
