...
import { Check, ChevronRight, Cpu, Upload } from 'lucide-react'
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-3">
        ...
        <div className="pt-1">
          <button
            type="button"
            onClick={onOpenAlgoDrawer}
            className="flex w-full items-center justify-center gap-1.5 rounded-xl border border-dashed border-[var(--border)] bg-[var(--bg-surface)] py-2 text-xs font-medium text-[var(--text-secondary)] transition-all hover:border-[var(--accent)] hover:text-[var(--accent)]"
          >
            <Upload className="h-3.5 w-3.5" />
            <span>{t('studio.uploadAlgo', { defaultValue: '上传新算法包' })}</span>
          </button>
        </div>
      </div>
    </aside>
