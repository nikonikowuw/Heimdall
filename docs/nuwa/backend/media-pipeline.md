...
- **证据流选择边界（告警可提升，抓拍不可）**：告警证据允许经上述换算取主码流高清帧（PRD 承诺 1080P/4K 证据）。
  **通行抓拍证据恒取该任务的分析码流**，禁止跨流提升或按 PTS 回溯主码流：抓拍的价值在于人工复核时能确认
  「图与检测框就是同一个人同一刻」，换算偏差带来的错配会直接毁掉这条证据；同轴时码流本就是同一条连接，
  提升无收益。实现上 `execute_capture_actions` 只走内存峰值候选
  （[`CandidateEvidence`](../../../crates/pipeline/src/snapshot.rs)）与当帧分析帧直出两条路，
  `image_pts_ms` 对抓拍恒可比。
