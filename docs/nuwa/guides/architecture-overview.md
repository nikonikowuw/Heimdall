...
```text
有效分析码流 (sub/main/auto 回退) → 硬件解码 → 原生 FrameRef → 门控 → 算法包按自身契约预处理/推理 → 后处理/跟踪/规则判定
双流模式主码流 → 裸 NALU RingBuffer → 告警时按需解码 → 全景图与 10% 扩边特写
主码流常驻分析模式 → 已解码 FrameRef 环 → 告警时按 PTS 零解码复用高保真帧
业务事件 → 有界写入缓冲 → SQLite / 证据文件 → HTTP 查询与稀疏 WS 通知
```

- `main` 模式和无可用子码流时的 `auto` 回退常驻解码有效主码流；`sub` 模式常驻解码子码流，双流时主码流不常驻解码。
- 宿主不为所有算法预设统一模型输入尺寸。算法包直接接收原生 `FrameRef`，自行选择预处理尺寸；需要宿主按能力准备输入时走算法实例能力协商。
...
- **下沉共享契约**：跨 crate 领域类型收敛于 `types`，不引入 IO/平台 SDK；跨算法包后处理（如 `algo_sdk::cv::postprocess` 的 DFL 解码、量化工具）统一收敛至 SDK 工具库，算法包只做参数配置。
- **杜绝过度设计**：通用逻辑在重复出现 3 次以上或多调用方边界明确时再抽象，一次性简单逻辑留在本地。
- **严格单向依赖**：单 crate 私有逻辑留在该 crate；前端共用 UI/工具放 `web/src/components/` 或 `lib/`，不为复用制造循环或反向依赖。
