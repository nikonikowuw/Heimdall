...
//!
//! 专为开发者在本地环境下快速验证 RK3576 RKNN 检测效果与模型精度设计。
//! 支持从 .env / 环境变量加载调试配置，
...
fn main() {
    eprintln!("run_local (RKNN) 仅支持在 Linux/Rockchip 平台上运行");
}
...
mod linux_run {
    ...
    use general_detection::postprocess::{
    ...
    pub fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let model_path = get_env_str("MODEL_PATH", "model/yolov8n-640x384-rk3576.rknn", &env_map);
        ...
        if is_fallback {
            println!(
            "[Pipeline Mode] ⚠️  开发调试回退路径 (debug_cpu_fallback_path) | 未检测到物理 RKNN NPU 驱动 (librknnrt.so) 或当前为非 Rockchip 架构"
        );
        } else {
            println!(
                "[Pipeline Mode] 🚀 常驻推理主路径 (infer_fast_path) | RKNN NPU 硬件加速已就绪"
            );
        }
        if !rga_hw {
            println!("[RGA Hardware]  ⚠️  RGA 2D 加速未就绪，预处理采用 CPU Letterbox 回退");
        } else {
        ...
        if is_stress {
            println!("============================================================");
            println!("  RK3576 RKNN 算法包高负荷持续压力测试 (Stress Test)");
            println!("============================================================");
        ...
        if benchmark {
            ...
            let mode_desc = if is_fallback {
                "⚠️ debug_cpu_fallback_path 模拟回退路径"
            } else {
                "🚀 infer_fast_path 真实硬件加速路径"
            };
            ...
            if is_fallback {
                println!("  [说明] 当前环境非 RK3576 实体板端，Inference 耗时仅反映回退数据闭环传递耗时（真实 RK3576 NPU 推理耗时通常在 6~12 ms 左右）");
            }
            println!(
