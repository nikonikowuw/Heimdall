    ...
    fn default() -> Self {
        Self {
            detector: ModelManifest {
                path: "model/yolov8n-face-640x384_mixed_face.rknn".to_string(),
                sha256: String::new(),
            ...
            embedder: ModelManifest {
                path: "model/edgeface_xs_gamma_06_rk3576_fp16.rknn".to_string(),
                sha256: String::new(),
                input: ModelInputManifest {
                    width: 112,
                    height: 112,
...
impl LoadedPackage {
    pub fn load(package_root: &Path) -> Result<Self, AlgoError> {
        ...
        if manifest.manifest_version != 1
            ...
            || !matches!(manifest.platform_id.as_str(), "linux-rknn" | "rknn-rk3576")
        {
            return Err(AlgoError::ModelLoad {
                reason: format!(
