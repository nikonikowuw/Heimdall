# RK3588 人脸识别算法包 (FaceLiVT & SCRFD Face Recognition)

基于 Rockchip RK3588 平台的工业级人脸识别算法包，集成 **SCRFD** 混合精度人脸检测器（含高精度 5 点关键点）与 **FaceLiVTv2** 512 维特征提取器（Super 8×8 空间对齐，0 CPU 回退）。
...
- `model/yolov6n_384x640_rk3588_i8.rknn`: YOLOv6n COCO 人体检测模型（640×384 INT8，直出 4 通道坐标，无 DFL 开销）
- `model/scrfd_500m_384x640_rk3588_mixed.rknn`: 视频流人脸检测与 5 点关键点模型（640×384 混合精度，消除 0.5 截断）
- `model/scrfd_2.5g_bnkps_640x640_rk3588_mixed.rknn`: 底库注册人脸检测与 5 点定位模型（640×640 混合精度）
- `model/facelivtv2_m_realistic_final_super_rk3588_fp16.rknn`: 512 维人脸特征提取模型（112×112 纯 FP16 FaceLiVTv2-M，监控场景退化自蒸馏微调版，Super 8×8，0 CPU 回退）
- `model/facelivtv2_l_realistic_final_super_rk3588_fp16.rknn`: 512 维人脸特征提取模型（112×112 纯 FP16 FaceLiVTv2-L，12×MHLA 大容量模型）
...
单进程内同一算法包目录只创建一个 RKNN worker（`heimdall-rk3588-face-npu` 线程），其内常驻
detector / embedder 两组 context；**所有通道实例共享该 worker**。由此产生三条硬约束：
...
RUST_LOG=face_recognition_rk3588=debug,algo_sdk=info <进程>
```
...
根 workspace 不包含算法包）：
```bash
MANIFEST=algo-packages/rknn/rk3588/Cargo.toml
cargo check   --manifest-path $MANIFEST -p face-recognition-rk3588-rknn --all-targets
cargo nextest run --manifest-path $MANIFEST -p face-recognition-rk3588-rknn
cargo clippy  --manifest-path $MANIFEST -p face-recognition-rk3588-rknn --all-targets -- -D warnings
```

在具备物理 RK3588 NPU 的硬件设备上运行：
```bash
# 硬件前向集成测试
cargo nextest run --manifest-path algo-packages/rknn/rk3588/Cargo.toml \
  -p face-recognition-rk3588-rknn --run-ignored all -- --nocapture

# 本地端到端自测工具
cargo run --manifest-path algo-packages/rknn/rk3588/Cargo.toml -p face-recognition-rk3588-rknn \
  --bin face_recognition_rk3588_rknn_run_local -- testimage.jpg --loops 20
```
