    ...
    pub fn load(package_root: &Path) -> Result<Self, AlgoError> {
        ...
        let person_detector_path = env
            .resolve_model_path(
                ...
                "model/yolov6n_384x640_rk3588_i8.rknn",
            )
            .or_else(|_| {
            ...
            .or_else(|_| {
                env.resolve_model_path(
                    &root,
                    "PERSON_DETECTOR_MODEL_PATH",
                    "model/yolov8n-640x384-rk3588.rknn",
                )
            })
            .or_else(|_| {
                env.resolve_model_path(
        ...
        let detector_path = env
            .resolve_model_path(
                &root,
                "DETECTOR_MODEL_PATH",
                "model/scrfd_500m_384x640_rk3588_mixed.rknn",
            )
            .or_else(|_| {
                env.resolve_model_path(
                    &root,
            ...
            .or_else(|_| {
                env.resolve_model_path(
                    &root,
                    "DETECTOR_MODEL_PATH",
                    "model/scrfd_2.5g_bnkps_640x384_rk3588_mixed.rknn",
                )
            })
            .or_else(|_| {
                env.resolve_model_path(
        ...
        let registration_detector_path = env
            .resolve_optional_model_path(
                &root,
                "REGISTRATION_DETECTOR_MODEL_PATH",
                "model/scrfd_2.5g_bnkps_640x640_rk3588_mixed.rknn",
            )
            .or_else(|_| {
                env.resolve_optional_model_path(
                    &root,
                    "REGISTRATION_DETECTOR_MODEL_PATH",
