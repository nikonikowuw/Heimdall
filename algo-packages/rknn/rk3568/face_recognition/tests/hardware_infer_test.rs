//! RK3568 真实 NPU 硬件前向推理与模型初始化集成测试
//!
//! 在具备 RKNN NPU 硬件和 librknnrt.so 的目标板（RK3568）上运行：
//! `cargo test -p face-recognition-rk3568-rknn -- --ignored`

use std::path::Path;

use face_recognition_rk3568::align::align_face;
use face_recognition_rk3568::{cosine_similarity, prepare_detector_input_for, shared_models};
