...
impl RknnRuntime {
    ...
    pub fn load(package_root: &Path) -> Result<Arc<Self>, AlgoError> {
        let candidates = [
            ...
            std::path::PathBuf::from("/opt/rknn-toolkit2/rknpu2/lib/librknnrt.so"),
        ];
...
impl RknnSession {
    ...
    pub fn new(
        ...
        if let Some(set_core_mask) = runtime.rknn_set_core_mask {
            // RK3568 物理上为单核 NPU。若驱动返回 -13 (Operation Not Supported) 则忽略，回退默认调度。
            // SAFETY: ctx 已成功初始化，0 是 RKNN_NPU_CORE_AUTO 掩码。
            let core_status = unsafe { set_core_mask(ctx, 0) };
            if core_status != RKNN_SUCC && core_status != -13 {
                // SAFETY: 当前 context 仍由本函数独占，失败路径成对销毁。
                unsafe { (runtime.rknn_destroy)(ctx) };
                return Err(AlgoError::ModelLoad {
                    reason: format!("rknn_set_core_mask(0) 失败，错误码: {core_status}"),
                });
            }
            if core_status == -13 {
                tracing::info!("RK3568 单核 NPU 无需设置 core_mask，使用默认核心调度");
            }
        } else {
            tracing::warn!("librknnrt 缺少 rknn_set_core_mask，使用默认单核调度");
        }
