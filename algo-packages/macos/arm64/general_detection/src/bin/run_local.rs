...
fn main() {
    eprintln!("run_local (CoreML) 仅支持在 macOS 平台上运行");
}
...
mod macos_run {
    ...
    use general_detection_coreml::config::InstanceConfig;
    use general_detection_coreml::plugin::GeneralDetector;
    use general_detection_coreml::postprocess::parse_and_unmap_detections;
    ...
    fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for i in 0..loops {
            ...
            if benchmark {
                ...
                let (buf, mode) =
                    AppleCvEngine.letterbox(&safe_frame, 640, 384, [114, 114, 114])?;
