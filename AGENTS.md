...
- **多消费者分发隔离与 GOP 恢复契约**：单路物理流支持多客户端与 AI 管道复用；分发层必须使用每消费者独立有界 Mailbox 隔离，慢读或断线消费者严禁反压阻塞物理接入与正常消费端。网络积压或源流重连时，必须以完整 GOP 快照（`Replay`）为对齐恢复边界，严禁向下游注入不可解码的残缺 P/B 帧。
- **纯视频硬件管线与音频隔离红线**：RTSP 音频仅在接入层解析并按需分发（支持伴生独立音频通道 `?audio=true&video=false`）；前端视频播放主通道强制保持纯视频（`hasAudio: false`），杜绝因无音频/非 AAC 摄像头导致 MSE 轨道饥饿死锁；硬件视觉解码引擎（VPU/DVPP/MPP）与主码流证据环（`RingBuffer`）严格面向纯 Annex-B 视频 NALU，严禁将音频包或非视频数据送入图像硬解管线。主码流证据环写入权归 `pipeline` 内部独占订阅，严禁在 `api` 路由中并发越权推流。
...
- **算法包沙箱与归档安全**：算法包统一遵循标准 C ABI 虚拟函数表；上传解压原生支持 `.tar.gz` (`.tgz`)、`.tar` 与 `.zip` 格式自动识别，解压过程必须强校验剔除绝对路径与包含 `..` 的路径组件（严防 Tar/Zip-Slip 漏洞），经六步沙箱物理自检通过后方可加载至运行时。

### 前端运行时

- Zustand 只保存客户端 UI 状态及 WebSocket 连接状态/有界增量缓冲；普通服务端资源通过专用数据获取 hook 管理。选择器保持细粒度，返回对象/数组时使用 `useShallow`。
- 视频播放器必须与非相关状态隔离并保持稳定 props；音频启用采用伴生独立播放器，静音/开音切换不得中断或重建主视频通道；高频检测框和实时流数据使用 `useRef`、`requestAnimationFrame`、Worker 或 Canvas，不用 React state 驱动高频 DOM 重排。
...
有对应平台 SDK 的环境再额外运行 `cargo clippy --all-targets --all-features -- -D warnings` 及平台测试；硬件依赖测试必须显式标记为 `#[ignore]`，开发机 `cargo nextest run` 仍须全绿。
