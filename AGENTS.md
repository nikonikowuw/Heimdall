...
Argus 仓库级协作入口，适用于 AI 智能体与工程师。本文只保留跨层、不可妥协的规则；分层实现细节以 `.trellis/spec/` 为准。
...
- 不持锁执行 IO、FFI 或 `.await`；跨线程转移帧所有权，不复制帧。文件、fd、buffer pool 租约和模型句柄必须有 RAII 生命周期；写盘必须有保留策略和磁盘水位保护。
...
- 事件缓冲有界，WebSocket 全局复用并指数退避重连；所有 UI 文本走 i18n，样式使用主题 token/CSS 变量，不硬编码颜色或提交调试日志。
