...
- **纯视频硬件管线与音频隔离红线**：RTSP 音频仅在接入层解析并在预览端按需合流（`?audio=true`）；硬件视觉解码引擎（VPU/DVPP/MPP）与主码流证据环（`RingBuffer`）严格面向纯 Annex-B 视频 NALU，严禁将音频包或非视频数据送入图像硬解管线。主码流证据环写入权归 `pipeline` 内部独占订阅，严禁在 `api` 路由中并发越权推流。
