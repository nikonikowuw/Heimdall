...
4. `<task-path>/api.md` if present — API contract for frontend+backend tasks (verify contract conformance; do not modify it)
...
2. **Review against task artifacts** — does the diff satisfy `prd.md` (and `design.md` / `api.md` / `implement.md` if present)?
