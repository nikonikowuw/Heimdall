...
4. `<task-path>/api.md` if present — API contract for frontend+backend tasks (Scope / Conventions / Shared models / Endpoints / Auth); do not modify it
5. `<task-path>/implement.md` if present — execution plan
6. `.trellis/spec/` — project-wide guidelines (load only what is relevant to the diff you are about to write)
